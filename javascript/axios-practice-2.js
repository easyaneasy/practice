...
async function productReviewsRating(id){
    const res = await axios.get(`${BASE_URL}/products/${id}`);
    const reviews = res.data.reviews;
    const rating = reviews.map((product) => product.rating);
    const sum = rating.reduce((acc, curr) => acc + curr, 0);
    const avg = sum/rating.length;
    console.log(avg);
}
productReviewsRating(10);
...
async function searchProducts(title){
    const res = await axios.get(`${BASE_URL}/products/search?q=${title}`);
    console.log(res.data.total);
}
searchProducts("phone");
searchProducts("glass");
